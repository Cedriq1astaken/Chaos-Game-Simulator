...
public class GlidingBoxLacunarity {
    ...
    public static void main(String[] args) {
        // Create a simple 2D test dataset (Sierpinski-like pattern)
        double[][] testData = generateRotatedSquareTestData2D(16, 45);

        // Initialize lacunarity calculator (2D, grid size 2^4 = 16)
        ...
        double[] lacunarityValues = calculator.calculateLacunarity(testData, boxSizes);
    ...
    /**
     * Generates a simple 2D test dataset
     */
    private static double[][] generateTestData2D() {
        List<double[]> points = new ArrayList<>();
        Random rand = new Random(42); // Fixed seed for reproducibility

        // Generate some clustered points
        for (int cluster = 0; cluster < 3; cluster++) {
            double centerX = rand.nextDouble();
            double centerY = rand.nextDouble();

            for (int i = 0; i < 50; i++) {
                double x = centerX + rand.nextGaussian() * 0.05; // Reduced spread for better clustering
                double y = centerY + rand.nextGaussian() * 0.05; // Reduced spread for better clustering
                points.add(new double[]{x, y});
            }
        }
        // Add some noise points to ensure not all bins are empty
        for (int i = 0; i < 20; i++) {
            points.add(new double[]{rand.nextDouble(), rand.nextDouble()});
        }
    ...
    private static double[][] generateSolidSquareTestData2D(int s_grid) {
        List<double[]> points = new ArrayList<>();
        // Assuming a square that fills the grid from 0 to s_grid-1
        // We can place a point at the center of each grid cell.
        // The effective range for binning would be 0 to 1 for each dimension.

        double step = 1.0 / s_grid; // Size of each grid cell in normalized coordinates

        for (int i = 0; i < s_grid; i++) {
            for (int j = 0; j < s_grid; j++) {
                // Place a point roughly in the center of each cell
                double x = (i * step) + (step / 2.0);
                double y = (j * step) + (step / 2.0);
                points.add(new double[]{x, y});
            }
        }
        return points.toArray(new double[0][]);
    ...
    private static double[][] generateRotatedSquareTestData2D(int s_grid, double angleDegrees) {
        List<double[]> points = new ArrayList<>();
        double step = 1.0 / s_grid;

        // Center of rotation (center of the normalized square)
        double centerX = 0.5;
        double centerY = 0.5;
        ...
        // Generate points for the unrotated square
        List<double[]> unrotatedPoints = new ArrayList<>();
        for (int i = 0; i < s_grid; i++) {
            for (int j = 0; j < s_grid; j++) {
                double x_unrotated = (i * step) + (step / 2.0);
                double y_unrotated = (j * step) + (step / 2.0);
                unrotatedPoints.add(new double[]{x_unrotated, y_unrotated});
            }
        }

        // Rotate each point
        for (double[] point : unrotatedPoints) {
            double x = point[0];
            double y = point[1];

            // Translate point so center of rotation is at origin
            double translatedX = x - centerX;
            double translatedY = y - centerY;

            // Rotate translated point
            double rotatedTranslatedX = translatedX * cosAngle - translatedY * sinAngle;
            double rotatedTranslatedY = translatedX * sinAngle + translatedY * cosAngle;

            // Translate point back
            double x_rotated = rotatedTranslatedX + centerX;
            double y_rotated = rotatedTranslatedY + centerY;

            points.add(new double[]{x_rotated, y_rotated});
        }
        return points.toArray(new double[0][]);
    }
}